...
//
//    private static final int BATCH_SIZE = 5_000;   // rows per INSERT
//
 ...
 *
 *  WHY CHUNKS OF 100,000?
 *  ─────────────────────
 *  - Keeps memory usage flat (~100k rows in RAM at a time, not 10M)
 *  - Allows resume: if the JVM crashes at chunk 47, you can see from logs
 *    exactly where it stopped and re-run with adjusted TARGET_MESSAGES
 *  - Each chunk logs a progress line so you can monitor in real time
 *  - Backfill per chunk means conversations.last_message_id stays correct
 *    even if you kill mid-run and inspect the API
 * ══════════════════════════════════════════════════════════════════════════
 */
@Slf4j
@Component
